...
class DemoPinchRotate: UIViewController {
    ...
    override func loadView() {
        ...
        let pinch = UIPinchGestureRecognizer(target: self, action: #selector(DemoPinchRotate.onPinch(_:)))
        girl!.addGestureRecognizer(pinch)
        
        let rotate = UIRotationGestureRecognizer(target: self, action: #selector(DemoPinchRotate.onRotate(_:)))
