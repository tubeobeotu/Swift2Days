...
class ThrowBall: UIViewController {
    ...
    override func loadView() {
        ...
        let pan = UIPanGestureRecognizer(target: self, action: #selector(ThrowBall.panIt(_:)))
        ball?.addGestureRecognizer(pan)
       
        timer = NSTimer.scheduledTimerWithTimeInterval(0.1, target: self, selector: #selector(ThrowBall.loop(_:)), userInfo: nil, repeats: true)
        timer?.fire()
    }
