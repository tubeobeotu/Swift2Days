...
class CachingImage: UITableViewController {
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if let image = imageCache.objectForKey(imageLinkArray[indexPath.row]) as? UIImage
        {
        ...
        else
        {
            ...
            cell.img.downloadImageFrom(link: imageLinkArray[indexPath.row], contentMode: .ScaleToFill, completetion: { (image) in
                self.imageCache.setObject(image, forKey: self.imageLinkArray[indexPath.row])
            })
        }
        return cell
