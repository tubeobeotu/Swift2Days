...
import GRMustache
import WebKit
open class PDFPreview: UIViewController, WKUIDelegate, WKNavigationDelegate {
                           ...
                           selectedImages: NSArray)
    {
        ...
        let template = try! GRMustacheTemplate(fromResource: "Document", bundle: Bundle(for: PDFPreview.self))
//        let template = try! Template(named: paramCollection[kPageOrientation] as! Int == 0 ? "Document" : "DocumentLanspace")
    ...
    func getDataFrom(_ table: String) -> Any
    {
        ...
        data = NSMutableDictionary(dictionary: dataHTML.getDataFrom(Bundle(for: PDFPreview.self).path(forResource: "demoHTML", ofType: "html")!, table: table))
